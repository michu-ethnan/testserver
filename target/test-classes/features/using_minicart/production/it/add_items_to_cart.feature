Feature: Add items to cart IT
  ...
  Scenario: Should add product from product page to minicart
    Given that Mindy has opened product page of a Giardino on IT
    When she tries to add it to cart
    Then she should see popup with "added to cart_IT" message

  Scenario: Should add product from product page to minicart
    Given that Mindy has clicked a product from a certain category on IT
    When she tries to add it to cart
    Then she should see popup with "added to cart_IT" message