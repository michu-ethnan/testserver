Feature: Add items to cart SK
  ...
  Scenario: Should add product from product page to minicart
    Given that Mindy has opened product page of a hrnček on SK
    When she tries to add it to cart
    Then she should see popup with "added to cart_SK" message

  Scenario: Should add product from product page to minicart
    Given that Mindy has clicked a product from a certain category on SK
    When she tries to add it to cart
    Then she should see popup with "added to cart_SK" message