Feature: Add items to cart CZ
  ...
  Scenario: Should add product from product page to minicart
    Given that Mindy has opened product page of a Zahrada on CZ
    When she tries to add it to cart
    Then she should see popup with "added to cart_CZ" message

  Scenario: Should add product from product page to minicart
    Given that Mindy has clicked a product from a certain category on CZ
    When she tries to add it to cart
    Then she should see popup with "added to cart_CZ" message